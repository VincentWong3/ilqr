...
int main() {
    ...
    Q_fast *= 1e3;
    Eigen::MatrixXd R_fast = Eigen::MatrixXd::Identity(1, 1) / 1.0;
    ...
    for (int i = 0; i <= num_points; ++i) {
        ilqr_node_list.push_back(std::make_shared<NewLatBicycleNodeInner<LinearConstraints<4, 1, 10>>>(L, dt, 0.001, v, 0.2, goal_list_fast[i], Q_fast, R_fast, constraints_obj));
    }
