...
int main() {
    ...
    std::array<Eigen::Matrix<double, 4, 1>, 2> state_bounds;
    state_bounds[0] << -1000, -1000, -2 * M_PI, -10;
    state_bounds[1] << 1000, 1000, 2 * M_PI, 10;

    std::array<Eigen::Matrix<double, 1, 1>, 2> control_bounds;
    control_bounds[0] << -0.2;
    control_bounds[1] << 0.2;

    BoxConstraints<4, 1> constraints_obj(state_bounds[0], state_bounds[1], control_bounds[0], control_bounds[1]);
    ...
    for (int i = 0; i <= num_points; ++i) {
        ilqr_node_list.push_back(std::make_shared<NewLatBicycleNode<BoxConstraints<4, 1>>>(L, dt, 0.001, v, goal_list_fast[i], Q_fast, R_fast, constraints_obj));
    }
