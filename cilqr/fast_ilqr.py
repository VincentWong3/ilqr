...
class FastILQR:
    ...
    def linearized_initial_guess(self):
        ...
        for node in self.ilqr_nodes:
            node.constraints_obj.lambda_ = np.zeros(node.constraints_obj.constraint_dim)
            node.mu = 1.0
    ...
    def backward(self):
        ...
        for t in reversed(range(horizon)):
            ...
            Quu_inv = np.linalg.inv(Quu + np.eye(control_dim) * 0.0)  # Regularization
    ...
    def update_lambda(self):
        for node in self.ilqr_nodes:
            node.constraints_obj.update_lambda()

    def update_mu(self, gain):
        for node in self.ilqr_nodes:
            node.constraints_obj.update_mu(node.constraints_obj.mu * gain)

    def compute_constrain_violation(self):
        violation = -1.0
        for node in self.ilqr_nodes:
            one_constrain = node.constraints_obj.max_violation(node.state, node.control)
            violation = np.maximum(violation, one_constrain)
        return violation
